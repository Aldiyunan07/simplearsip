	...
	*/
	class TCPDF {
		...
		public function Write($h, $txt, $link='', $fill=0, $align='', $ln=false, $stretch=0, $firstline=false, $firstblock=false, $maxh=0) {
			...
			$shy_replacement = 45;
			$shy_replacement_char = $this->unichr((int) $shy_replacement);
			...
			while ($i < $nb) {
				...
				if ($c == 10) { // 10 = "\n" = new line
					...
					// \p{Lo} or \p{Other_Letter}: a Unicode letter or ideograph that does not have lowercase and uppercase variants.
					// \p{Lo} is needed because Chinese characters are packed next to each other without spaces in between.
					if (($c != 160) AND (($c == 173) OR preg_match($this->re_spaces, $this->unichr((int) $c)))) {
		...
		public function UTF8ArrSubString($strarr, $start='', $end='') {
			...
			$string = '';
			for ($i=$start; $i < $end; ++$i) {
				$string .= $this->unichr((int) $strarr[$i]);
			}
			return $string;
		}
	...
	Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
		...
		protected function putHtmlListBullet($listdepth, $listtype='', $size=10) {
        	...
        	switch ($listtype) {
				...
				case 'lower-greek': {
					$textitem = $this->unichr((int) 945 + $this->listcount[$this->listnum] - 1);
					break;
				}
				/*
						...
						for ($j = 0; $j < $pattern_length; ++$j) {
							...
							if (($pattern[$j] >= 48) AND ($pattern[$j] <= 57)) {
								...
								if (!isset($hyphenword[$zero]) OR ($hyphenword[$zero] != $pattern[$j])) {
									$hyphenword[$zero] = $this->unichr((int) $pattern[$j]);
								}
								++$digits;
							}
						}
